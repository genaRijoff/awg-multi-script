...
# модуле не проверяется и ничего не даёт. Действует только когда задан
# HeaderProtectionKey, то есть на 3.x; на 2.0 низкие S остаются как были.
AWG_HP_MIN_S=12
...
#
# Прежнее обоснование этой правки («совпавшая длина возвращает наблюдателю
# тип пакета») при перепроверке не подтвердилось: совпадение длин тип как раз
# СКРЫВАЕТ, а не выдаёт. Настоящая польза мельче и в другом: initiation и
# response идут в разные стороны, так что по направлению они различимы при
# любых S, а вот одинаковая длина запроса и ответа делает обмен симметричным,
# тогда как у WireGuard он асимметричен (148 против 92). Разведение оставляет
# асимметрию неповторимой: длины не совпадают ни с эталонными 148/92/64, ни
# между собой. Эффект небольшой, но и цена нулевая.
#
